...
def InterpretLine(line):
    ...
    subject = elements[3].split('\'>')[1][:-9] # split and remove trailing '</a'
    author = elements[4][:-11] # remove trailing '</td></tr>\n'
    
    # return a dictionary for each line
    return {'Modified': modified, 'Module': module, 'URL': url, 'Subject': subject, 'Author': author}
...
def SpecifyContribution(data, colname, day_start=None, day_end=None, max_names=5, plot=True, saveplot=None):
    ...
    data['Days_since'] = (data['Modified'].values - day_start) / np.timedelta64(1,'D')
    ...
    col_conts = sorted(Counter(data[colname].values).items(), key=lambda x:x[1], reverse=True)
...
#         box = ax.get_position()
        ...
        for name in cols[:max_names]:
            data_to_plot = data[data[colname] == name][['Days_since',colname + 'Counts']].values
            ax.plot(np.concatenate((data_to_plot[:,0],[0])), np.concatenate((data_to_plot[:,1],[0])), label=name, linewidth=2)
        ax.grid(True)
        # add the legend
#         ax.legend(bbox_to_anchor=(1.05, 1), loc=1)
        ...
        if (day_end - day_start) / np.timedelta64(1,'D') <= 100:
            xticks = [str(year) + month + day for year in np.arange(day_start.astype(object).year,day_end.astype(object).year+1)                      for month in ['-01','-02','-03','-04','-05','-06','-07','-08','-09','-10','-11','-12']                      for day in ['-01','-11','-21']]
            xticklabels = [day + ('\n' + month + ('\n' + str(year)) * (month == 'Jan.')) * (day == '01')                           for year in np.arange(day_start.astype(object).year,day_end.astype(object).year+1)                          for month in ['Jan.','Feb.','Mar.','Apr.','May','Jun.','July','Aug.','Sept.','Oct.','Nov.','Dec.']
                          for day in '01',' 11',' 21']
        elif (day_end - day_start) / np.timedelta64(1,'D') <= 300:
            xticks = [str(year) + month + day for year in np.arange(day_start.astype(object).year,day_end.astype(object).year+1)                      for month in ['-01','-02','-03','-04','-05','-06','-07','-08','-09','-10','-11','-12']                      for day in ['-01']]
            xticklabels = [month + ('\n' + str(year)) * (month == 'Jan.')                           for year in np.arange(day_start.astype(object).year,day_end.astype(object).year+1)                          for month in ['Jan.','Feb.','Mar.','Apr.','May','Jun.','July','Aug.','Sept.','Oct.','Nov.','Dec.']]
        else:
            xticks = [str(year) + month + day for year in np.arange(day_start.astype(object).year,day_end.astype(object).year+1)                      for month in ['-01','-04','-07','-10']                      for day in ['-01']]
            xticklabels = [month + ('\n' + str(year)) * (month == 'Jan.' or (month == 'Apr.' and year == 2013))                           for year in np.arange(day_start.astype(object).year,day_end.astype(object).year+1)                           for month in ['Jan.','Apr.','July','Oct.']]

        # add those labels
        ax.tick_params(axis='both', labelsize=20)
        ax.set_xticks((np.array(xticks,dtype=np.datetime64) - day_start) / np.timedelta64(1,'D'))
